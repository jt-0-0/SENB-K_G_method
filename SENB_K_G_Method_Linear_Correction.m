% ----------------------------------------------------------------------- %
% ------ Code to determine K_IC and G_IC (with linear correction) ------- %
...
% This code is closely related to SENB_K_G_Method.m but for specimen data 
% will replace the load v displacement graph to the left of the first 
% selected gradient point, with a linear section, of average gradient,
% passing through (0,0).
% It is recommended to use this code only in certain situations (as 
% explained in the help guide)

% The code allows for a linear correction to the start of the load vs.
% extension data for both sample and compliance such that it removes 
% any initial load ramping errors.

% For the specimen, the selected gradient for the linear correction, is the 
% same as that used to calculate P5.
...
%   would like to truncate test data
...
% gradient, this will be used make the linear correction and also to find 
% the Load 5 line. 
...
% Pre-allocate variables in the loop for computing efficiency
...
Pmax_corr = zeros(1,No_Spec);
Pmaxlength_corr = zeros(1,No_Spec);
...
% for i=5 % To run for one specimen, specify i as a single value, i.e. i=1 for specimen 1
    ...
    % Correct Load-Ext curve
    % find y intercept for line with grad(i) that passes through M1
    y_intercept_corr(i) = Load(M_x1(i))-grad(i)*Ext(M_x1(i));
    x_intercept_corr(i) = -y_intercept_corr(i)/grad(i);
    
    % create linear array of elements that lines up to gradient select
    % point 1
    Ext_corr= -1:0.001:Ext(M_x1(i));
    Ext_corr=Ext_corr'-x_intercept_corr(i);
    Load_corr=grad(i)*Ext_corr;
    
    % concatenate with the original graph to meet with M point 1
    Ext_corr=cat(1,Ext_corr,Ext(M_x1(i)+1:end)-x_intercept_corr(i));
    Load_corr=cat(1,Load_corr,Load(M_x1(i)+1:end));
    
    % Delete Load-Ext data below 0,0
    [~,origin] = min(abs(Ext_corr));
    Ext_corr=Ext_corr(origin:end);
    Load_corr=Load_corr(origin:end);
    
    % Set new Pmax and Pmaxlength
    [Pmax_corr(i),Pmaxlength_corr(i)] = max(Load_corr);
    
    % Plot corrected load v extension
    plot(Ext_corr, Load_corr,'color',col{8});
    
    % Plot the gradient fit on the graph
    fplot(@(x)grad(i)*x-y_intercept_corr(i)/grad(i), [0 max(Ext_corr)],'--','color',col{9});
    ...
    % 0.95 gradient line
    Ext_Load5=cat(1,Ext_corr,(Ext_corr(2:end)+ max(Ext_corr))); % create array twice as long as extension
    Load5=grad5(i)*Ext_Load5; % produce load5 line data
    
    % Find intersection between 5% and load ext plot, when is Load5 > Load?
    Loaddiff = Load5(1:max(size(Load_corr))) - Load_corr;
    is = find(Loaddiff>0);
    ...
    plot(Ext_corr(1:P5length(i)),Load5(1:P5length(i)),'--','color',col{1});
    
    % Find the load at the point of crossing (Pq)
    Pq(i) = grad5(i)* Ext_corr(P5length(i)); 

    % Create variables for truncated load and extension
    Load_trunc=Load_corr;
    Ext_trunc=Ext_corr;
    
    if  P5length(i) < Pmaxlength_corr(i) % If Pq happens before Pmax, use Pq    
        P(i) = Pq(i);

        % Truncate load ext data beyond P5length nd plot markers denoting
        % Pmax and P = Pq
        plot(Ext_corr(Pmaxlength_corr(i)),Load_corr(Pmaxlength_corr(i)),'o','MarkerSize',4,'color','black') % plot P max small
        Load_trunc((P5length(i)+1):end) = [];
        Ext_trunc((P5length(i)+1):end) = [];
        plot(Ext_corr(P5length(i)),Load5(P5length(i)),'*','MarkerSize',10, 'color','black')
        legend('Location', 'northwest','NumColumnsMode','manual','NumColumns',2)
        legend('Test Data','Corrected Test Data','S','0.95S','P_{max}','P = P_q')
        ...
        % Truncate load ext data beyond P5length nd plot markers denoting
        % Pq and P = Pmax
        plot(Ext_corr(P5length(i)),Load5(P5length(i)),'o','MarkerSize',4, 'color','black') % plot Pq small
        Load_trunc((Pmaxlength_corr(i)+1):end) = [];
        Ext_trunc((Pmaxlength_corr(i)+1):end) = [];  
        plot(Ext_corr(Pmaxlength_corr(i)),Load_corr(Pmaxlength_corr(i)),'*','MarkerSize',10, 'color','black'); 
        legend('Location', 'northwest','NumColumnsMode','manual','NumColumns',2)
        legend('Test Data','Corrected Test Data','S','0.95S','P_q','P = P_{max}')
        
    end
        
    % Add axis limits for plot
    if max(Ext)>max(Ext_corr)
       xlim([0 (max(Ext)*1.05)]);
    else
       xlim([0 (max(Ext_corr)*1.05)]);
    end
    ...
    if P5length(i) < Pmaxlength_corr(i) % If Pq happens before Pmax, use Pq    
        ...
        legend('Compliance','Corrected Test Data','P_{max}','P = P_q')
        lgd.NumColumns = 2;
        ...
        plot(Ext_corr(Pmaxlength_corr(i)),Load_corr(Pmaxlength_corr(i)),'*','MarkerSize',10, 'Color','black');
        legend('Location', 'southeast','NumColumnsMode','manual','NumColumns',1)
        legend('Compliance','Corrected Test Data','P_q','P = P_{max}')
    end
